...
void draw() {
  ...
  for (int i=0; i<locationTable.getRowCount(); i++) {
    ...
    if(highlightedMunicipality==municipalityName){
          noFill();
          stroke(0,0,0);
          rect(screenX + xTextOffset, screenY-20,screenX + 10*municipalityName.length(), screenY+20);
          fill(0, 255, 0);
  ...
  text("number of island in this diection:",1600,y+10);
  y = y+30;
  for(int i = 1;i<=10;i++){
    color goodColor = lerpColorLab( color(230,230,230), color(255,0,0),((15-i))/10);
          float[] c = colorToLab(goodColor);
          // stroke(255-municipalityList.size()*25.5);
          stroke(c[0],c[1],c[2]);
          strokeWeight(i/2);
          line(1500,y,1600,y+20);
          text(i,1450,y);
  ...
  if(selectedMunicipality!=""){
    noStroke();
  float mouselon = panZoomMap.longitudeToScreenX(mouseX);
  float mouselat = panZoomMap.latitudeToScreenY(mouseY);
  // print(mouseX,mouseY,mouselon,mouselat,"\n");
  TableRow ddd= locationTable.findRow(selectedMunicipality, "Municipality");
  float angcallon = panZoomMap.longitudeToScreenX(ddd.getFloat("Longitude"));
  float angcallat = panZoomMap.latitudeToScreenY(ddd.getFloat("Latitude"));
  float dirangle = paafuDirections.getAngleFromTo(angcallon,angcallat,mouseX,mouseY);

  String dirname = paafuDirections.getDirectionFromTo(angcallon,angcallat,mouseX,mouseY);
  
  // float dirangle = paafuDirections.getAngle(dirname);
  // print(dirname,dirangle,"\n");
  fill(0,255,0,50);
  arc(angcallon,angcallat,1000,1000,radians(dirangle-5.625),radians(dirangle+5.625),PIE);
  fill(255,0,0);
  strokeWeight(3);
  text(dirname,mouseX,mouseY);
  }
  if(selectedMunicipality!=""){
  HashMap<String, ArrayList<String>> kdata = kinshipTies.getKinshipTies(selectedMunicipality);
  for (String paafuDirection : kdata.keySet()) {
       ArrayList<String> municipalityList = kdata.get(paafuDirection);
       if (municipalityList.size() > 0) {
        //  println("From " + selectedMunicipality + " in direction " + paafuDirection + ":");
         String temp = selectedMunicipality;
         for (String municipality2 : municipalityList) {
          TableRow org= locationTable.findRow(temp, "Municipality");
          TableRow des= locationTable.findRow(municipality2, "Municipality");
          float orgscreenX = panZoomMap.longitudeToScreenX(org.getFloat("Longitude"));
          float orgscreenY = panZoomMap.latitudeToScreenY(org.getFloat("Latitude"));
          float desscreenX = panZoomMap.longitudeToScreenX(des.getFloat("Longitude"));
          float desscreenY = panZoomMap.latitudeToScreenY(des.getFloat("Latitude"));
          color goodColor = lerpColorLab( color(230,230,230), color(255,0,0),(15-municipalityList.size())/10);
          float[] c = colorToLab(goodColor);
          // stroke(255-municipalityList.size()*25.5);
          stroke(c[0],c[1],c[2]);
          strokeWeight(municipalityList.size()/2);
          line(orgscreenX,orgscreenY,desscreenX,desscreenY);
          temp = municipality2;
